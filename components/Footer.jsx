...
export default function Footer() {
	...
	const { styles, companyInfo, navigation, support, contact, subscription, socialMedia, mobile, desktop } = footerData;
		...
		<footer className={styles.footerClass}>
			...
			<div className={styles.mobileContainerClass}>
				...
				<div className='text-left mb-6'>
					...
					/>
					<p className={companyInfo.descriptionClass}>{companyInfo.description}</p>
				</div>
							...
							<ul className={navigation.mobileListClass}>
								{navigation.links.map((link) => (
									<li
										...
										<a
											href={link.href}
											className={navigation.linkClass}>
											{link.name}
										</a>
									</li>
								))}
				...
				<div className='mb-6'>
					<form
						...
						<button
							...
							disabled={isSubmitting}>
							<span className='text-sm'>{isSubmitting ? 'Sending...' : subscription.buttonText}</span>
							<img
					...
					{message && <p className={`text-sm mt-2 ${message.includes('Thank you') ? 'text-green-600' : 'text-red-600'}`}>{message}</p>}
				</div>
			...
			<div className={styles.desktopContainerClass}>
				...
				<div className={`mb-6 md:mb-0 ${desktop.leftSectionWidth}`}>
					...
					<div>
						...
						<p className={`${companyInfo.descriptionClass} mb-4 max-w-xs`}>{companyInfo.description}</p>
						<p className={companyInfo.copyrightClass}>{companyInfo.copyright}</p>
					</div>
				</div>
				...
				<div className={`mb-6 md:mb-0 ${desktop.contactWidth}`}>
					...
					<div className='space-y-3'>
						{contact.contacts.map((contactItem) => (
							<div
								key={`contact-${contactItem.id}`}
								className='text-xs'>
								<span className='font-medium block'>{contactItem.type === 'phone' ? 'Call: ' : 'Email: '}</span>
								<a
									href={contactItem.href}
									className={contact.linkClass}>
									{contactItem.value}
								</a>
							</div>
						))}
				...
				<div className={desktop.socialWidth}>
					<form
						...
						<div className='Group88 w-96 h-14 relative'>
							...
							<button
								...
								disabled={isSubmitting}>
								<span className='text-white text-sm font-normal'>{isSubmitting ? '...' : subscription.buttonText}</span>
								<img
									src={subscription.polygonIcon}
									alt='Polygon'
									className='w-3 h-4'
					...
					{message && (
						<p className={`text-sm mt-2 ml-32 ${message.includes('Thank you') ? 'text-green-600' : 'text-red-600'}`}>{message}</p>
					)}
