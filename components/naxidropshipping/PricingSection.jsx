    ...
    <section className="relative">
      ...
      <div className="relative max-w-full md:py-10 text-center bg-[#0F255A] overflow-visible">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:space-x-4 mb-6 px-4">
          ...
          <a
            href="/support"
            className="md:mb-0 mb-12 text-white text-xs md:text-lg hover:text-gray-300"
          >
      ...
      {/* Pricing Section */}
      <div className="relative max-w-full mt-8  text-center bg-gray-100 overflow-visible">
        <h2 className="text-3xl md:text-5xl font-bold mb-6 pt-12 text-black">
          Pricing
        </h2>
        ...
        <div className="md:hidden relative px-6 mb-8">
          ...
          <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory">
            {plans.map((plan, index) => (
              <div
                key={index}
                className={`${plan.gradient} min-w-[280px] p-6 rounded-3xl shadow-lg text-white snap-center flex flex-col`}
              >
                <Image
                  src={plan.icon}
                  alt={`${plan.title} icon`}
                ...
                <p className="text-sm leading-relaxed flex-grow">
                  Naxi's strategically located UAE warehouses are your ultimate
                  advantage. We store, pack, and ship everything for you,
                  ensuring lightning-fast delivery across all seven emirates.
                </p>
              </div>
            ))}
          </div>
        ...
        {/* Desktop Grid Layout */}
        <div className="hidden md:grid grid-cols-3 gap-10 md:gap-16 px-6 mb-9 md:px-50">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`${plan.gradient} p-8 md:p-12 rounded-3xl shadow-lg text-white transition-transform duration-300 hover:scale-105`}
            >
              <Image
                src={plan.icon}
                alt={`${plan.title} icon`}
              ...
              <p className="text-sm md:text-md leading-relaxed">
                Naxi's strategically located UAE warehouses are your ultimate
                advantage. We store, pack, and ship everything for you, ensuring
                lightning-fast delivery across all seven emirates.
              </p>
            </div>
          ))}
        </div>

        {/* <p className="mt-10 text-base md:text-xl underline font-semibold pb-20 cursor-pointer text-black hover:text-blue-600"></p> */}
      </div>
    </section>
  );
}
